RSpec.describe Availability do
  ...
  describe '#initialize' do
    context 'AbstractAvailability' do
      it 'should not be able to instantiate' do
  ...
  describe 'serialization' do
    let(:availability_factory) do
      lambda do |suffix|
        Availability.send("every_other_#{suffix}", duration: 1.hour, start_time: Date.today)
      end
    end
    ...
    include_examples 'InstanceVariableComparability#<=>' do
      let(:factory) { lambda { |hash| Availability.every_other_hour **hash } }
    ...
    include_examples 'InstanceVariableComparability#==' do
      let(:factory) { lambda { |hash| Availability.every_other_day **hash } }
    ...
    it "can be marshalled to YAML" do
      availabilities.each do |_, availability|
        ...
        expect(yaml).to include "- !ruby/object:Availability::Exclusion"
        expect(yaml).to include "rule: !ruby/object:Availability::Exclusion::Rule::BeforeDate"
        expect(yaml).to include "rule: !ruby/object:Availability::Exclusion::Rule::BeforeTime"
  ...
  describe '#interval=' do
    it 'updates the residue when the interval changes' do
      availability = Availability.create(duration: 45.minutes, interval: 30, start_time: beginning + 10.days)
  ...
  describe '#last_occurrence' do
    ...
    context 'when availability has a stops_by attribute' do
      context 'daily' do
        subject { Availability.daily start_time: Date.yesterday, duration: 4.hours, stops_by: expected_last_occurrence }
      ...
      context 'weekly stops on same date as expected last occurrence' do
        subject { Availability.weekly start_time: Time.new(1973, 5, 15), duration: 4.hours, stops_by: expected_last_occurrence }
      ...
      context 'weekly stops by some point in a different residue class' do
        subject { Availability.weekly start_time: Time.new(1973, 5, 15), duration: 4.hours, stops_by: expected_last_occurrence + 4.days }
        let(:expected_last_occurrence) { Time.new(1973, 5, 29) }
      ...
      context 'monthly stops on same date as expected last occurrence' do
        subject { Availability.monthly start_time: beginning, duration: 8.hours, stops_by: expected_last_occurrence }
      ...
      context 'monthly stops by some point in a different residue class' do
        subject { Availability.monthly start_time: beginning, duration: 8.hours, stops_by: expected_last_occurrence + 23.days }
        let(:expected_last_occurrence) { (beginning + 3.months).to_time }
      ...
      context 'yearly stops on same date as expected last occurrence' do
        subject { Availability.yearly start_time: beginning, duration: 1.hour, stops_by: expected_last_occurrence + 1.day }
      ...
      context 'yearly stops by some point in a different residue class' do
        subject { Availability.yearly start_time: beginning, duration: 1.hour, stops_by: expected_last_occurrence + 3.months + 2.days }
        let(:expected_last_occurrence) { (beginning + 10.years).to_time }
  ...
  describe '#includes?' do
    ...
    context 'when the frequency is daily' do
      ...
      it 'should process availabilities of different intervals on the same day when applicable' do
        availability_1 = Availability.create(duration: 45.minutes, interval: 10, start_time: beginning)
        availability_2 = Availability.create(duration: 45.minutes, interval: 8, start_time: beginning + 2.days)
        availability_3 = Availability.create(duration: 45.minutes, interval: 15, start_time: beginning + 5.days)
        availability_4 = Availability.create(duration: 45.minutes, interval: 10, start_time: beginning + 5.days)
  ...
  describe '#next_occurrence' do
    context 'when the frequency is daily' do
      context 'when the availability is every 21 days and starts 2 days after the beginning' do
        let(:availability) { Availability.create(duration: 45.minutes, interval: 21, start_time: beginning + 2.days) }
      ...
      context 'when the availability is every 10 days and starts 4 days after the beginning' do
        let(:availability) { Availability.create(duration: 45.minutes, interval: 10, start_time: beginning + 4.days) }
    ...
    context 'when the frequency is monthly' do
      context 'when the availability is every 4 months and starts at the beginning' do
        let(:availability) { Availability.create(duration: 45.minutes, frequency: :monthly, interval: 4, start_time: beginning) }
    ...
    context 'when the frequency is yearly' do
      context 'and the availability is every 3 years and starts at the beginning' do
        let(:availability) { Availability.create(duration: 2.hours, frequency: :yearly, interval: 4, start_time: beginning) }
    ...
    context 'when there are exclusions' do
      ...
      context 'daily' do
        ...
        let(:availability) {
          Availability.daily(duration: 1.hour, start_time: the_day_before, stops_by: the_third_day_after).tap do |a|
            a.exclusions = Availability::Exclusion.all_day july_fourth
          end
        }
      ...
      context 'weekly' do
        ...
        let(:availability) {
          Availability.weekly(duration: 1.hour, start_time: the_week_before, stops_by: the_third_week_after).tap do |a|
            a.exclusions = Availability::Exclusion.all_day july_fourth
          end
        }
  ...
  describe '#next_n_occurrences' do
    context 'when the frequency is daily' do
      context 'when the availability is every 7 days and starts 31 days after the beginning' do
        let(:availability) { Availability.create(duration: 45.minutes, interval: 7, start_time: beginning + 31.days) }
      ...
      context 'when the availability is every 40 days and starts 11 days after the beginning' do
        let(:availability) { Availability.create(duration: 45.minutes, interval: 40, start_time: beginning + 11.days) }
    ...
    context 'when the frequency is monthly' do
      context 'when the availability is every 7 months and starts 3 months after the beginning' do
        let(:availability) { Availability.create(duration: 45.minutes, frequency: :monthly, interval: 7, start_time: beginning + 3.months) }
    ...
    context 'performance' do
      let(:availability) { Availability.daily(duration: 45.minutes, interval: 7, start_time: beginning + 3.months) }
    ...
    context '#corresponds_to?' do
      let(:availability) { Availability.daily(duration: 45.minutes, start_time: beginning + 3.months) }
      let(:similar_availability) { Availability.daily(duration: 45.minutes, start_time: beginning + 3.months) }
