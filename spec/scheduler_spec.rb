...
RSpec.describe Scheduler do
  ...
  context '#allow?' do
    ...
    it 'does not allow an event request that starts before the availability frequency' do
      expect(bobs_schedule.allow? one_hour_slot_per_week(at: [2016, 4, 4, 9], frequency: 4)).to eq false
      expect(bobs_schedule.allow? one_hour_slot_per_week(at: [2016, 4, 4, 9], frequency: 5)).to eq false
    end
  end
end
